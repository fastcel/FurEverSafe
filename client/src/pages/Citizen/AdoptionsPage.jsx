...
export default function AdoptionsPage() {
    ...
    const handleConfirmCancel = async () => {
        try {
            ...
            setAdoptions(prev => prev.filter(a => a.application_id !== targetApplicationId));
        } catch (err) {
            ...
            <div className="w-full px-8 py-8">
                <div className="flex justify-between items-center mb-8">
                    ...
                    <div className="relative">
                        <button
                            ...
                            className="bg-[#C2185B] text-white px-8 py-2 rounded-sm font-bold border-2 border-black"
                        >
                            Filter
                        </button>
                ...
                {loading ? (
                    ...
                    <div className="space-y-5">
                        {adoptions.length > 0 ? (
                            adoptions.map(app => (
                                <div key={app.application_id} className="bg-[#DED9C4] border-2 border-black p-6 flex items-center justify-between rounded-lg shadow-sm hover:shadow-md transition-all">
                                    <div className="flex items-center gap-8">
                                        ...
                                        <div className="space-y-1">
                                            <h3 className="text-2xl font-black text-[#6A1B9A]">{app.pet_name}</h3>
                                            <p className="text-base font-medium text-[#6A1B9A]">
                                                {app.breed} <span className="mx-2">•</span> {app.age}
                                            </p>
                                            <p className="text-base font-bold text-[#6A1B9A]">{app.city}</p>
                                    ...
                                    <div className="flex flex-col items-end gap-3">
                                        <span className={`px-4 py-1 rounded-full font-black text-sm border-2 border-black ${getStatusColor(app.status)}`}>
                                            {app.status.charAt(0).toUpperCase() + app.status.slice(1)}
                                        </span>
                                        {app.status === 'pending' && (
                                            <button
                                                onClick={() => handleCancelClick(app.pet_name, app.application_id)}
                ...
                {showCancelModal && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
                        <div className="bg-[#EDEFD7] p-8 border-2 border-black rounded-lg max-w-sm text-center shadow-xl">
                            ...
                            <h3 className="text-xl font-black text-[#C2185B] mb-4">Are you sure you want to cancel?</h3>
                            <p className="text-sm mb-8 text-gray-600">
                                Are you sure you want to withdraw your application for <strong>{targetPet}</strong>?
                                This will remove you from the waiting list and notify the NGO.
                            </p>
