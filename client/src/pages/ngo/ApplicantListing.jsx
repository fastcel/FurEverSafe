...
export default function ApplicantListing({ pet, onBack }) {
    ...
    // fetch all applicants for this pet
    useEffect(() => {
    ...
    try {
        ...
        // re-fetch applicants to get updated statuses and counts
        const res = await axios.get(
            `http://localhost:5000/api/adoption/ngo/pets/${pet.pet_id}/applications`,
            { headers: { Authorization: `Bearer ${token}` } }
        );
        ...
        // if approved, go back so NGOAdoptions re-fetches and moves pet to accepted
        if (action === 'accept') {
            onBack();
        }
                                ...
                                >
                                    ...
                                    {applicant.status !== 'pending' && (
                                        <span className={`block text-xs mt-1 ${
                                            applicant.status === 'approved' ? 'text-green-300' : 'text-red-300'
                                        }`}>
                        ...
                        <div className="flex-1 bg-[#EDEFD7] p-8 overflow-y-auto">
                            {detailLoading ? (
                                ...
                                <>
                                    ...
                                    {selectedApplicant?.status !== 'pending' && (
                                        <div className={`text-center font-black text-xl mb-8 py-3 rounded-lg border-2 border-black ${
                                    ...
                                    <div className="grid grid-cols-2 gap-6 text-base">
                                        {[
                                            ...
                                            { label: "Preferred Contact", value: details.preferred_contact_method },
                                            { label: "House Type", value: details.house_type?.replace(/_/g, ' ') },
                                            {
                                                label: "Monthly Income",
                                                value: details.monthly_income_range
                                                    ? details.monthly_income_range.replace(/_/g, ' ').replace(/\s+/g, ' - ')
                                                    : '—'
                                                },
                                                {
                                                label: "Monthly Budget",
                                                value: details.monthly_budget_range
                                                    ? details.monthly_budget_range.replace(/_/g, ' ').replace(/\s+/g, ' - ')
                                                    : '—'
                                            },
                                            { label: "Pet Alone Hours", value: details.pet_alone_hours?.replace(/_/g, ' ') },
                                            { label: "Has Children", value: details.has_children ? 'Yes' : 'No' },
                                            { label: "Other Pets", value: details.other_pets?.join(', ') || 'None' },
                                        ].map(({ label, value }) => (
                                            <div key={label} className="bg-white border-2 border-black rounded-lg p-4">
                                                <p className="text-xs font-bold text-gray-500 uppercase mb-1">{label}</p>
                                                <p className="font-bold text-[#6A1B9A]">{value || '—'}</p>
                                            </div>
                                        ))}
                                    </div>
                                    ...
                                    <div className="mt-6 bg-white border-2 border-black rounded-lg p-4">
                                        ...
                                        <p className="font-bold text-[#6A1B9A] leading-relaxed">{details.motivation || '—'}</p>
                                    </div>
                                </>
                            ) : (
                                <p className="font-bold text-gray-500">Select an applicant to view details.</p>
                            )}
                        </div>
                    </div>
                )}
