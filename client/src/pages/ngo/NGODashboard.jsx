...
function mapRowToPet(row) {
  ...
  const ageYears = String(
    row.age != null && row.age !== "" ? Number(row.age) : 0,
  );
  return {
    pet_id: row.pet_id,
    name: row.name,
...
export default function NGODashboard() {
  ...
  const loadPets = useCallback(async () => {
    ...
    try {
      ...
      console.error(err);
      setError(
        err.response?.data?.error || "Failed to load pets from the server.",
      );
      setRows([]);
    } finally {
      setLoading(false);
  ...
  const filteredSortedRows = useMemo(() => {
    ...
    if (q) {
      list = list.filter((row) => {
        const hay =
          `${row.name || ""} ${row.breed || ""} ${row.city || ""}`.toLowerCase();
    ...
    if (typeFilter) {
      list = list.filter(
        (row) =>
          (row.pet_type || "").toLowerCase() === typeFilter.toLowerCase(),
      );
    }
    return list;
          ...
          <div className="relative">
            ...
            {showFilter && (
              <div className="absolute right-0 mt-2 w-72 bg-[#EDEFD7] border-2 border-black z-30 p-4 shadow-lg">
                <div className="font-bold border-b border-black pb-2 mb-3">
                  Animal type
                </div>
