...
export default function NGOAdoptions() {
    ...
    try {
        setLoading(true);
        const token = localStorage.getItem('token');
        const res = await axios.get('http://localhost:5000/api/adoption/ngo/pets', {
            headers: { Authorization: `Bearer ${token}` }
        });
    ...
    return (
        <ApplicantListing
            ...
            onBack={() => {
                ...
                fetchPets(); // ← re-fetch so tabs update
            }}
        />
    );
}
            ...
            <div className="w-full px-8 py-8">
                ...
                {loading ? (
                    ...
                    <div className="space-y-5">
                        {activeTab === 'ongoing' ? (
                            ongoingPets.length > 0 ? ongoingPets.map(pet => (
                                <NGOAdoptionItem
                                    key={pet.pet_id}
                                    data={{
                                        id: pet.pet_id,
                                ...
                                />
                            )) : <p className="font-bold text-gray-500">No ongoing applications.</p>
                        ) : (
                            acceptedPets.length > 0 ? acceptedPets.map(pet => (
                                <div key={pet.pet_id} className="bg-[#DED9C4] border-2 border-black p-6 flex items-center justify-between rounded-lg shadow-sm">
                                    <div className="flex items-center gap-8">
                                        <img src={pet.image_url || 'https://placecats.com/300/200'} className="w-28 h-28 border-2 border-black object-cover rounded-md" alt="" />
                                        <div className="space-y-1">
                                            <h3 className="text-2xl font-black text-[#6A1B9A]">{pet.name}</h3>
                                            <p className="text-base font-medium text-[#6A1B9A]">{pet.breed} • {pet.age}</p>
                                            <p className="text-base font-bold text-[#6A1B9A]">{pet.city}</p>
                                        </div>
                                    </div>
                                    <button
                            ...
                            )) : <p className="font-bold text-gray-500">No accepted adoptions yet.</p>
                        )}
                    </div>
                )}
